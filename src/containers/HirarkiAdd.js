...
function HirarkiAdd() {
  ...
  const _send = () => {
    ...
    insertData('http://127.0.0.1:3333/v1/hirarkis', _fields)
      .then(() => {
        message.success('Berhasil menambahkan hirarki!')
        history.push('/hirarki')
        value._fetch({ offset: 0 })
      })
    ...
    <div style={{ padding: 50 }}>
      ...
      <Row type='flex' justify='end' style={{ marginTop: 30 }}>
        <Col sm={3} style={{ marginRight: 10 }}>
          <Link to='/hirarki'>
            <Button type='primary' block>Kembali</Button>
          </Link>
        </Col>
        <Col sm={3}>
          <Button type='danger' block onClick={_send} loading={loading}>Simpan</Button>
        </Col>
      </Row>
    </div>
  )
